...
sys.path.append("C:\\Users\\Stally Mugs\\PycharmProjects\\HelloWorld\\pythonProject\\synthlogic\\main\\synth.py")
from interfaces.ext_input.midi import MidiInterface
from synthlogic.main.interfaces.processing.envelope import Env
from synthlogic.main.interfaces.processing.filter import LowPass, Allpass
import synthlogic.main.oscillator as osc
from structures.value import DataInterface


class Synth:
    def __init__(self, rate=44100, chunk_size=1024, gain=0.5, fade_seq=0, gui_enabled=False):
    ...
    def render(self):
        ...
        while self.running:
            ...
            if self.gui_enabled:
                ...
                fc_Lp = self.data_interface.ft_cutoff.vale_log
                M_delay = int(self.data_interface.ft_reverb.value_log)
                self.env.settings(self.data_interface.env_attack.value_log,
                                  self.data_interface.env_decay.value_log,
                                  self.data_interface.env_sustain.value_log,
        ...
        #   chunks = np.append(chunks, self.chunk)0
        #   if start >+ 44100:
        #       self.toggle()
        #
        # print('ended')
        # wavefile.write('recorded.wave', 44100, chunks)
